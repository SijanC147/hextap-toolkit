...
`)
	...
	if submodules == "" || submodules == manifest.SubmodulesNone {
		...
		result.WriteString("A fine-grained personal access token is the intended shape, and it selects repositories under **one** resource owner. If this repository and any private submodule sit under different owners, no fine-grained token can cover both, and nothing here constrains a submodule URL to this owner. In that case use a credential that can span owners, a GitHub App installation token or a classic token, and scope it as narrowly as that type allows. Moving the submodule under one owner is the cheaper fix where it is possible.\n\n")
	}
