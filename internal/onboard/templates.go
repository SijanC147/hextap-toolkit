...
`)
	...
	if submodules == "" || submodules == manifest.SubmodulesNone {
		...
		result.WriteString("If the remainder spans owners, a fine-grained token cannot express it. Moving those repositories under one owner is the only fix that keeps the narrow credential, and nothing in the manifest constrains a submodule URL, so that is a choice about layout rather than something the toolkit enforces. Otherwise use a **classic** personal access token belonging to a user who can read all of them, and understand the cost: its `repo` scope covers every repository that user can reach rather than the ones you list, so give it a short expiry and rotate it. A GitHub App installation token cannot substitute, because an installation belongs to one account and its token expires after an hour, while this workflow reads one statically stored secret.\n\n")
	}
