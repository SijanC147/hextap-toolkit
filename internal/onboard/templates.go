...
`)
	...
	if submodules == "" || submodules == manifest.SubmodulesNone {
		...
		result.WriteString("```\n\n")
		result.WriteString("It needs **Contents read on this repository and on each submodule repository, and nothing else**. Include this repository: `actions/checkout` uses that token for the primary clone as well as for the submodule fetches, so a token scoped to the submodules alone fails the clone before reaching one. It is a different credential from the tap publisher token and the two must not be conflated.\n\n")
		result.WriteString("A fine-grained personal access token is the intended shape, and it selects repositories under **one** resource owner. If this repository and any private submodule sit under different owners, no fine-grained token can cover both, and nothing here constrains a submodule URL to this owner.\n\n")
		result.WriteString("**Move the submodule under one owner if you can.** That keeps the fine-grained token and its narrow scope, and it is the only option here that does not widen the credential.\n\n")
		result.WriteString("If you cannot, the credential must be a **classic** personal access token belonging to a user who can read every repository involved. A GitHub App installation token does not work: an installation belongs to one account, so its token cannot reach repositories owned by another, and it expires after an hour while this workflow reads one statically stored secret. Understand what you are accepting with a classic token: its `repo` scope covers every repository that user can reach, not the ones you list, so it is far broader than this release needs and it is worth a short expiry and a calendar reminder to rotate it.\n\n")
	}
