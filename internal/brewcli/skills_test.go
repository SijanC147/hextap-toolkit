...
func TestSkillsInstallAndStatusCLIUseTemporaryUserHome(t *testing.T) {
	...
	if code != 0 || stderr != "" || !strings.Contains(stdout, "CURRENT claude-code discovered_by=claude-code,cursor installed=1.2.0 available=1.2.0 action=NONE ") {
...
func TestSkillsStatusDefaultsToCompleteUserInventoryAndSupportsJSON(t *testing.T) {
	...
	for _, expected := range []string{
		"CURRENT agents+codex discovered_by=agents,codex,cursor installed=1.2.0 available=1.2.0 action=NONE ",
		"NOT_INSTALLED claude-code discovered_by=claude-code,cursor installed=- available=1.2.0 action=INSTALL ",
		"NOT_INSTALLED cursor discovered_by=cursor installed=- available=1.2.0 action=INSTALL ",
	} {
	...
	if document.Schema != 1 || document.Scope != "user" || len(document.Entries) != 3 || document.Entries[0].Agent != "agents+codex" || strings.Join(document.Entries[0].DiscoveredBy, ",") != "agents,codex,cursor" || document.Entries[0].InstalledVersion != "1.2.0" {
...
func TestSkillsUpgradeCLIPlansAndAppliesManagedForwardUpgrade(t *testing.T) {
	...
	if code != 0 || stderr != "" || !strings.Contains(stdout, "UPGRADE claude-code from=0.9.0 to=1.2.0 ") || strings.Contains(stdout, "backup=") {
	...
	if code != 0 || stderr != "" || !strings.Contains(statusOutput, "CURRENT claude-code discovered_by=claude-code,cursor installed=1.2.0 available=1.2.0 action=NONE ") {
